...
import carla
from agents.motion.local_planner import RoadOption
from agents.libs.misc import vector,draw_waypoints

class GlobalRoutePlanner(object):
    """
    This class provides a very high level route plan.
    TODO, the global oute planner should be able to do the following
    find the shortest path between a starting point and a destination
    """
    ...
    def trace_route(self, origin, destination, world=None, debug=False):
        """
                ...
                for waypoint in path[closest_index:]:
                    ...
                    if len(route)-i <= 2 and waypoint.transform.location.distance(destination) < 2*self._sampling_resolution:
                        ...
                        if closest_index > destination_index:
                            break
        if debug:
            waypoints, _ = list(zip(*route_trace))
            draw_waypoints(world, waypoints, 1.0, lifespan=120, is_line=True)
            # print(list(zip(*route_trace)))
        return route_trace, waypoints
