...
from agents.motion.local_planner import LocalPlanner, RoadOption
from agents.motion.mission_planner import GlobalRoutePlanner
from agents.tools.misc import (get_speed, is_within_distance,
                               get_trafficlight_trigger_location,
                               compute_distance)

from agents.tools.animate import Animation
...
class BasicAgent(object):
    ...
    def set_destination(self, end_location, start_location=None, animate=False):
        """
        ...
        route_trace, waypoints = self.trace_route(start_waypoint, end_waypoint)
        if animate:
            self._animation = Animation(waypoints)
    ...
    def trace_route(self, start_waypoint, end_waypoint):
        ...
        return self._global_planner.trace_route(start_location, end_location,self._world, debug=True)

    def run_step(self, debug=False, animate=False):
        ...
        if self._target_speed == 0.0 and vehicle_speed > 0.01:
            print(vehicle_speed)
            print(f'Vehicle location: {self._vehicle.get_location():.2f}')

        if animate:
            # call the animation, on the matplotlib
            self.animate()
        return control
    
    def animate(self):
        self._animation.run_step()
        pass
