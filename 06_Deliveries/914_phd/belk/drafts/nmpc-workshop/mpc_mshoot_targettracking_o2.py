...
class NMPCController:
    ...
    def _setup_mpc(self):
        ...
        P = ca.SX.sym('P', n_states+self.N*(n_ref)+n_states) # parameters (Initial and reference trajectory)
        ...
        for k in range(self.N+1):
            ...
            g=ca.vertcat(g, -ca.sqrt((X[0,k]-P[n_states+n_ref*self.N])**2 +(X[1,k]-P[n_states+n_ref*self.N+1])**2)+(0.05+P[n_states+n_ref*self.N+2]))
            # o2 radius 1.5 [4,6]
            # print(X[:,k])        
    ...
    def compute_control(self, mpciter):
        ...
        if self.x0[0] <= 3.5 or x_ref <=3.5:
            self.args['p'][-3:] = [1.5, 3, 0.25]
        else:
            self.args['p'][-3:] = [5, 5, 1.25]
        # print(self.args['p'][-3:])
...
def main():
    ...
    try:
        ...
        while (mpciter < (sim_time / nmpc.dt)):
