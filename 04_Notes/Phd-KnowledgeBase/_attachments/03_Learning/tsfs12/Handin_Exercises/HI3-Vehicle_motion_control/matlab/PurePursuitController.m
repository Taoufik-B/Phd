classdef PurePursuitController < PurePursuitControllerBase
    properties
        l;
        L;
        plan;
    ...
    methods
        function obj=PurePursuitController(l, L, path, goal_tol, pursuit_point_fig)
        % Create pure-pursuit controller object
        % 
        %    PurePursuitController(l, L, path[, goal_tol, pursuit_point_fig])
        %
        % Input arguments:
        ...
        %   pursuit_point_fig - Figure handle (default: false)            
            if nargin < 5
                pursuit_point_fig = false;
            end
            if nargin < 4
                goal_tol = 0.25;
            end
            obj = obj@PurePursuitControllerBase(pursuit_point_fig);
        ...
        function c = u(obj, t, w)
            ...
            obj.pursuit_plot(p_car, p_purepursuit);  % Included for animation purposes.
        end
