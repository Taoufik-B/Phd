...
rg = np.random.default_rng(
    seed=1891
)  # Random number generator used when simulating actions
...
def next_state(s, a, params):
    ...
    s_prim, r, _ = p_next_state[
        rg.choice(range(len(p_next_state)), p=[p_i[2] for p_i in p_next_state])
    ]
    return s_prim, r
...
Q = np.random.uniform(
    size=(n_rows, n_cols, 4)
)  # Number of rows x number of columns x number
# of actions
...
for k in range(nbr_iters):
    ...
    while not terminal_state:
        ...
        Q[s_curr[0], s_curr[1], action] = Q[s_curr[0], s_curr[1], action] + alpha * (
            r
            + gamma * np.max(Q[s_prim[0], s_prim[1]])
