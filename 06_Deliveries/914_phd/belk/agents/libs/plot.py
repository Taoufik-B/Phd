...
from agents.libs.misc import get_waypoints_xy, get_speed
...
class Path:
    ...
    def __init__(self, waypoints=None):
        ...
        if waypoints is None:
            ...
            with open('belk/agents/data/carla_trajectory.csv', newline='') as f:
...
class UpdateEgoVeh:
    def __init__(self, path, ego_veh):
        self.success = 0
        # self.prob = prob
        self.fig = plt.figure(layout="constrained")
        self.path = path
        # self.ego_imu = ego_imu
        self.ego_veh = ego_veh
        self.distance = 0
        self.ego_veh_previous_location = ego_veh.get_transform().location
        
        self.sfigs = self.fig.subfigures(1, 2, width_ratios=[3, 3])
        self.axs_left = self.sfigs[0].subplots()

        ((self.ax_r_0, ax_r_1),( ax_r_2,ax_r_3),( ax_r_4, ax_r_5)) = self.sfigs[1].subplots(3, 2)
        self.ego_x_history = deque(maxlen=len(self.path.x))
        self.ego_y_history = deque(maxlen=len(self.path.y))
        self.ego_speed_history = deque(maxlen=len(self.path.x))
        self.ego_distance_history = deque(maxlen=len(self.path.x))


        # Set up plot parameters
        # self.ax.set_xlim(0, 1)
        # self.ax.set_ylim(0, 10)
        self.axs_left.grid(True)


        # This vertical line represents the theoretical value, to
        # which the plotted distribution should converge.
        # self.axs_left.axvline(200, linestyle='--', color='black')
        self.ref,  = self.axs_left.plot(self.path.px, self.path.py, '--', color='orange')
        self.ego_cor,  = self.axs_left.plot([], [], '-', color='blue')
        self.ego_speed, = self.ax_r_0.plot([0.0, 1000], [0.0, 200], '-', color='blue')
        # ax_r_0.autoscale()
    ...
    def __call__(self, i):
        ...
        ego_veh_t = self.ego_veh.get_transform()
        ego_speed = get_speed(self.ego_veh)


        self.distance += ego_veh_t.location.distance(self.ego_veh_previous_location)
        self.ego_veh_previous_location=ego_veh_t.location
        # print(self.distance, ego_speed)

        self.ego_x_history.append(ego_veh_t.location.x)
        self.ego_y_history.append(ego_veh_t.location.y)
        self.ego_speed_history.append(ego_speed)
        self.ego_distance_history.append(self.distance)
        self.ego_speed.set_data(self.ego_distance_history, self.ego_speed_history)
        self.ego_cor.set_data(self.ego_x_history, self.ego_y_history)
        # self.ax_r_0.autoscale()
        return self.ref, self.ego_cor, self.ego_speed
...
def main():
    ...
    # fig, ax = plt.subplots()
    uego = UpdateEgoVeh(path)
    anim = FuncAnimation(uego.fig, uego, interval = 100, blit=True)
    plt.show()

    # gen_wp()
    # _ = FuncAnimation(fig, update, frames=gen_wp, init_func=lambda : setup(fig))
    pass
