...
class DQN_Agent:
    ...
    def act(self, s):
        ...
        """
        _, actions = self._Q(torch.tensor(np.array([s]), dtype=torch.float).to(self.device)).max(1)
    ...
    def Q(self, s):
        ...
        """
        return self._Q(torch.tensor(np.array([s]), dtype=torch.float).to(self.device)).data.cpu().numpy()
    ...
    def train(self, num_episodes, verbose=True, display=False):
        ...
        for k in range(num_episodes):
            ...
            while not done:
                ...
                s_next, r, term, trunc, _ = self.env.step(a)  # Act on the environment and record next state and reward
                done = term or trunc
                tot_reward += r
                if display:
