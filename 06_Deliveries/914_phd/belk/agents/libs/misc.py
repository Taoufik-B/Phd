...
def get_waypoints_xy(waypoints):
    x = []
    y = []
    _unique_waypoints = list(set(waypoints))
    for i, wpt in enumerate(waypoints):
        wpt_t = wpt.transform.location
        wpt_pre = waypoints[i-1].transform.location if i>0 else wpt_t
        wpt_conseq_is_equal = wpt_pre.x == wpt_t.x
        if wpt_conseq_is_equal:
            continue
        x.append(wpt_t.x)
        y.append(wpt_t.y) 
    return x,y
...
def write_trajectory_file(x_list, y_list):
    OUTPUT_FOLDER = "agents/data"
    file_name = os.path.join(OUTPUT_FOLDER, 'carla_trajectory.csv')

    with open(file_name, 'w') as trajectory_file: 
        trajectory_file.write("X-axis,Y-axis\n")
        for i in range(len(x_list)):
            trajectory_file.write('%3.3f, %3.3f\n' %\
                                  (x_list[i], y_list[i]))
