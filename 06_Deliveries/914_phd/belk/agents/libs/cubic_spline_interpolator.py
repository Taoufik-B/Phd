...
def generate_road_borders():
    pass
def generate_cubic_path(x: Any, y: Any, ds: float=0.05, bc_type: str='natural') -> Tuple[np.ndarray, np.ndarray]:
...
def calculate_spline_yaw(x: Any, y: Any, ds: float=0.05, bc_type: str='natural') -> np.ndarray:
    
    cs, s = initialise_cubic_spline(x, y, ds, bc_type)
    dx, dy = cs.derivative(1)(s).T
    return np.arctan2(dy, dx)

def calculate_spline_curvature(x: Any, y: Any, ds: float=0.05, bc_type: str='natural') -> np.ndarray:
...
def main():
    ...
    # dir_path = 'agents/data/waypoints_racetrack.csv'
    dir_path = 'agents/data/carla_trajectory.csv'
