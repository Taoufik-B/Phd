...
M = 50
val_class_idx = (
    rg.choice(left_class_idx, M, replace=False),
...
train_class_idx = (
    rg.choice(np.setdiff1d(left_class_idx, val_class_idx[0]), M),
...
def train_model(model, epochs, train_data, val_data):
    ...
    for k in range(epochs):
        ...
        print(
            f"loss: {train_loss:>.4f} - accuracy: {train_acc:>0.3f} - val_loss {val_loss:>.4f} - val_accuracy: {val_acc:>0.3f}"
        )
...
class IntentPredictionModel(nn.Module):
    def __init__(self, lr=1e-3):
        ...
        self.network = nn.Sequential(
            nn.Linear(41, 3), nn.ReLU(), nn.Linear(3, 3), nn.Softmax(1)
        )

        self.loss_fun = nn.CrossEntropyLoss()
        self.optimizer = torch.optim.Adam(
            params=self.network.parameters(), lr=lr, weight_decay=0
        )
...
for trajectory_idx in rg.choice(range(len(trajectories[0])), N):
    ...
    ax.plot(
        trajectory.Local_X,
        trajectory.Local_Y,
        color=colors[trajectory.Lane_ID.iloc[0]],
        lw=0.5,
    )
plot_road()
ax.set_xlabel("x [m]")
